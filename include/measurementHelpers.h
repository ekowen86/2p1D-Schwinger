...
void measWilsonLoops(const Complex gauge[LX][LY][2], int iter, param_t p){
  ...
  for (int smear_rad = 0; smear_rad <= loopMax; smear_rad++) {

      // smear the gauge field out to a given radius
      // int smear_steps = int(double(smear_rad) / p.wf_dt);

      int smear_steps = int(double(smear_rad * smear_rad) / p.wf_dt) / 4;
      if (smear_steps > 1000) break;

      // wilson flow the gauge field
      for (; wf_steps < smear_steps; wf_steps++) wilsonFlow(smeared, p.wf_dt);
	...
	for(int sizey = (sizex == 1 ? 1 : sizex-1); (sizey < loopMax && sizey <= sizex+1); sizey++) {
	  ...
	  name += "_hits" + to_string(wf_steps);
      name += "_" + to_string(sizex) + "_" + to_string(sizey);
	  constructName(name, p);
	  name += ".dat";
	  sprintf(fname, "%s", name.c_str());
	  fp = fopen(fname, "a");
	  fprintf(fp, "%.16e %.16e\n", real(wLoops[sizex][sizey]), imag(wLoops[sizex][sizey]));
	  fclose(fp);
	}
    }
...
double measTopCharge(const Complex gauge[LX][LY][2], param_t p){
  ...
  Complex smeared[LX][LY][2];
  copyLat(smeared, gauge);
  for (int step = 0; step < 500; step++) coolLink(smeared);
