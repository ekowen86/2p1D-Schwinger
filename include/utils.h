...
void printParams(param_t p) {
  ...
  cout << "Smearing: WF steps = " << p.wf_steps << endl;
  cout << "          WF dt = " << p.wf_dt << endl;
  cout << "          Cooling Steps = " << p.coolIter << endl;
#ifdef USE_ARPACK
    ...
    for(int i=0; i<iter; i++) {
        ...
        for(int x=0; x<LX; x++) {
            ...
            for(int y=0; y<LY; y++) {
                ...
                SmearedTmp[x][y][0] += alpha * Smeared[x][y][1] * Smeared[x][yp1][0] * conj(Smeared[xp1][y][1]);
                SmearedTmp[x][y][0] += alpha * conj(Smeared[x][ym1][1]) * Smeared[x][ym1][0] * Smeared[xp1][ym1][1];
                SmearedTmp[x][y][1] += alpha * Smeared[x][y][0] * Smeared[xp1][y][1] * conj(Smeared[x][yp1][0]);
...
Complex staple(Complex gauge[LX][LY][2], int x, int y, int mu) {
    ...
    int ym1 = (y - 1 + LY) % LY;
    if (mu == 0) {
        return gauge[x][y][1] * gauge[x][yp1][0] * conj(gauge[xp1][y][1])
            + conj(gauge[x][ym1][1]) * gauge[x][ym1][0] * gauge[xp1][ym1][1];
    } else {
        return gauge[x][y][0] * gauge[xp1][y][1] * conj(gauge[x][yp1][0])
            + conj(gauge[xm1][y][0]) * gauge[xm1][y][1] * gauge[xm1][yp1][0];
    }
}

// staple x is 0th, y is 1st.
void wilsonFlow(Complex gauge[LX][LY][2], double dt) {
    ...
    for (int x = 0; x < LX; x++) {
        for (int y = 0; y < LY; y++) {
            for (int mu = 0; mu < 2; mu++) {
                ...
                double z = -imag(w0[x][y][mu] * conj(s));
                z0[x][y][mu] = dt * z;
                w1[x][y][mu] = polar(1.0, (1.0/4.0) * z0[x][y][mu]) * w0[x][y][mu];
            }
        }
    }
    ...
    for (int x = 0; x < LX; x++) {
        for (int y = 0; y < LY; y++) {
            for (int mu = 0; mu < 2; mu++) {
                ...
                double z = -imag(w1[x][y][mu] * conj(s));
                z1[x][y][mu] = dt * z;
                w2[x][y][mu] = polar(1.0, (8.0/9.0) * z1[x][y][mu] - (17.0/36.0) * z0[x][y][mu]) * w1[x][y][mu];
            }
        }
    }
    ...
    for (int x = 0; x < LX; x++) {
        for (int y = 0; y < LY; y++) {
            for (int mu = 0; mu < 2; mu++) {
                ...
                double z = -imag(w2[x][y][mu] * conj(s));
                z2[x][y][mu] = dt * z;
                w3[x][y][mu] = polar(1.0, (3.0/4.0) * z2[x][y][mu] - (8.0/9.0) * z1[x][y][mu] + (17.0/36.0) * z0[x][y][mu]) * w2[x][y][mu];
            }
        }
    }
...
void coolLink(Complex gauge[LX][LY][2]){
